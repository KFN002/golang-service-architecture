...
func RunAudit(cfg config.Audit) error {
	...
	app, err := httpv1.NewAuditApp(httpv1.AppConfig{
		...
		Log:         log,
		Limiter:     limiter,
		Readiness:   readiness(store.Ping, rds.Ping, mq),
	}, server)
	if err != nil {
		return err
	}
	...
	g.Go(func() error {
		lc := net.ListenConfig{}
		lis, err := lc.Listen(gctx, "tcp", fmt.Sprintf(":%d", cfg.GRPCPort))
		if err != nil {
			return err
		}
	...
	g.Go(func() error {
		...
		stopGRPCGracefully(grpcSrv, constants.DefaultShutdownTimeout, log)
