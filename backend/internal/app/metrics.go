...
func registerOrchestratorMetrics(hub *httpv1.SSEHub) {
	promauto.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: nsCalc, Subsystem: "sse", Name: "clients",
...
func registerAgentMetrics(instance string, pool *workerpool.Pool) {
	...
	promauto.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: nsCalc, Subsystem: nsPool, Name: "workers", ConstLabels: labels,
		Help: "Current goroutine workers in the pool.",
	}, func() float64 { return float64(pool.Stats().Workers) })
	promauto.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: nsCalc, Subsystem: nsPool, Name: "busy", ConstLabels: labels,
		Help: "Workers currently computing.",
	}, func() float64 { return float64(pool.Stats().Busy) })
	promauto.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: nsCalc, Subsystem: nsPool, Name: "backlog", ConstLabels: labels,
		Help: "Tasks queued in the pool.",
	}, func() float64 { return float64(pool.Stats().Backlog) })
	promauto.NewCounterFunc(prometheus.CounterOpts{
		Namespace: nsCalc, Subsystem: nsPool, Name: "done_total", ConstLabels: labels,
...
func registerAuditMetrics(ing *audit.Ingestor, bh *bulkhead.Bulkhead) {
	promauto.NewCounterFunc(prometheus.CounterOpts{
		Namespace: nsAudit, Name: "accepted_total", Help: "Events accepted into the pipeline.",
	}, func() float64 { return float64(ing.Stats().Accepted) })
	promauto.NewCounterFunc(prometheus.CounterOpts{
		Namespace: nsAudit, Name: "deduplicated_total", Help: "Duplicate events dropped.",
	}, func() float64 { return float64(ing.Stats().Deduplicated) })
	promauto.NewCounterFunc(prometheus.CounterOpts{
		Namespace: nsAudit, Name: "flushed_total", Help: "Events durably flushed.",
	}, func() float64 { return float64(ing.Stats().Flushed) })
	promauto.NewCounterFunc(prometheus.CounterOpts{
		Namespace: nsAudit, Name: "flush_errors_total", Help: "Failed batch flushes.",
	}, func() float64 { return float64(ing.Stats().FlushErrors) })
	promauto.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: nsAudit, Name: "backlog", Help: "Events buffered awaiting flush.",
	}, func() float64 { return float64(ing.Backlog()) })
	promauto.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: nsAudit, Name: "ingest_in_flight", Help: "AMQP ingest bulkhead occupancy.",
	}, func() float64 { inFlight, _ := bh.Stats(); return float64(inFlight) })
	promauto.NewCounterFunc(prometheus.CounterOpts{
		Namespace: nsAudit, Name: "ingest_rejected_total", Help: "Bulkhead rejections (load shed).",
