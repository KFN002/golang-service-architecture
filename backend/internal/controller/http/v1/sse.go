...
// SubscribeFunc consumes a pub/sub channel, invoking fn per message.
type SubscribeFunc func(ctx context.Context, channel string, fn func(payload string)) error

// RunRedisBridge pumps the Redis events channel into the hub until ctx ends.
func (h *SSEHub) RunRedisBridge(ctx context.Context, subscribe SubscribeFunc, channel string) error {
